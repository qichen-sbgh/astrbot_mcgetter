...
- 一次查询全部已保存服务器：在线状态、延迟、版本、MOTD、在线玩家
- **并发查询**：多服并行拉取状态，单服超时后单独按离线处理
- **离线失败卡**：某台服查不到时仍输出结构化离线卡片（含上次成功时间），不影响其他服
...
- 🎮 **多服务器管理** - 支持添加、删除、查询多个 Minecraft 服务器
- 📊 **实时状态查询** - 在线状态、玩家数、延迟、版本、MOTD 等
- 👥 **玩家列表显示** - 显示当前在线玩家列表（sample / 可扩展完整名单）
- 🖼️ **图片化展示** - 霓虹玻璃等内置主题 + 离线失败卡
- 🎨 **颜色定制** - `/mccolor` 设置服务器名 / 玩家名颜色
- 🔒 **地址验证** - 自动验证服务器地址格式和连接性
- 📝 **群组独立** - 每个群组独立管理服务器列表与卡片主题
- 🧹 **自动清理** - 自动删除长时间未查询成功的服务器
- 🆔 **ID管理** - 基于 ID 的服务器管理，支持名称和 ID 双重操作
- 🕹️ **多主题 / 自定义模板** - 内置 neon/classic/dashboard/inventory/soft/compact，并支持数据目录自定义脚本
...
返回图片化状态信息，包含服务器名称/ID、在线状态、玩家数、版本、延迟、MOTD、在线玩家等。
多服会并行查询；单台失败时输出离线卡（可带上次成功时间），其余服务器正常出图。

自动清理：每次执行 `/mc` 时会自动清理长期未查询成功的服务器（默认 10 天，可在插件配置中调整）。
...
/mctem              # 查看当前主题与可用列表
/mctem list
/mctem neon         # 切换本群主题（群间互不影响）
```

内置主题：`neon`（默认）、`classic`、`dashboard`、`inventory`、`soft`、`compact`。
也可将自定义 `draw_image` 脚本放入插件数据目录的 `template/` 下，按文件名切换。

#### 颜色

```text
/mccolor server 主服 #00FFC8
/mccolor player Steve #FF55FF
/mccolor list
/mccolor clear server 主服
/mccolor clear player Steve
```

颜色格式：`#RRGGBB` / `#RGB` / `R,G,B`。
...
主题按**群**保存在群配置 JSON 的 `template` 字段；`/mctem` 只影响当前群。

1. 编写或下载自定义处理脚本，放入插件数据目录 `template/<名称>.py`
    脚本需满足以下条件:
    - 调用函数必须命名为 `draw_image`
    - 需包含以下参数:
...
**Q: 可以修改清理天数吗？**
A: 可以，在 AstrBot 插件配置中修改 `auto_cleanup_days`（默认 10）
...
- [GitHub Issues](https://github.com/QiChenSn/astrbot_mcgetter/issues)
...
- [ ] 玩家名称颜色随在线天数改变
- [ ] 服务器状态历史记录
- [x] 自定义图片主题 / 内置多主题 / 按群切换
- [x] 并发查询与离线失败卡
- [x] MOTD 展示
- [x] `/mccolor` 服务器名与玩家名颜色
- [ ] 定时自动查询功能
- [ ] 服务器分组管理
